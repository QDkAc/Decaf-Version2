...
import decaf.tacvm.exec.Memory.Block;
...
public final class Executor {
	...
	public class Intrinsic {
		...
		public int callIntrinsic(int index) {
			...
			case 0:
				ret = _Alloc();
				garbageCollector.setRvReferenceMemory(ret);
				return ret;
			case 1:
	...
	public class GarbageCollector {
		...
		private Map<Integer, Integer> heapReferenceStringTable;
		private Map<Integer, Integer> stackReferenceStringTable;
		Integer rvReferenceMemory, rvReferenceStringTable;

		public GarbageCollector(long period) {
			memoryReferenceCount = new HashMap<Integer, Integer>();
			heapReferenceMemory = new HashMap<Integer, Integer>();
			stackReferenceMemory = new HashMap<Integer, Integer>();
			stringTableReferenceCount = new HashMap<Integer, Integer>();
			heapReferenceStringTable = new HashMap<Integer, Integer>();
			stackReferenceStringTable = new HashMap<Integer, Integer>();
			rvReferenceMemory = rvReferenceStringTable = null;
		...
		void setRvReferenceMemory(Integer address) {
			clearRv();
		...
		void setStackReferenceMemory(Integer address, Integer content) {
		...
		void setHeapReferenceMemory(Integer address, Integer content) {
		...
		void setHeapReferenceStringTable(Integer address, Integer content) {
		...
		void clearRv() {
			if (rvReferenceMemory != null) {
				...
				if (memoryReferenceCount.get(rvReferenceMemory) == 0) {
					clearBlock(rvReferenceMemory);
					memory.dispose(rvReferenceMemory);
				}
			...
			if (rvReferenceStringTable != null) {
				...
				if (stringTableReferenceCount.get(rvReferenceStringTable) == 0) {
					System.out.println("Garbage Detected: String Table at adress " + rvReferenceStringTable);
				}
		...
		void clearStack(int address) {
			if (stackReferenceMemory.containsKey(address)) {
				Integer to = stackReferenceMemory.get(address);
			...
			if (stackReferenceStringTable.containsKey(address)) {
				...
				if (stringTableReferenceCount.get(to) == 0) {
					System.out.println("Garbage Detected: String Table at adress " + to);
				}
		...
		void clearHeap(int address) {
			if (heapReferenceMemory.containsKey(address)) {
				Integer to = heapReferenceMemory.get(address);
				memoryReferenceCount.put(to, memoryReferenceCount.get(to) - 1);
				if (memoryReferenceCount.get(to) == 0) {
					clearBlock(to);
					memory.dispose(to);
		...
		public void assignStackToHeap(int from, int to) {
			clearHeap(to);
			if (stackReferenceMemory.containsKey(from)) {
		...
		public void assignHeapToStack(int from, int to) {
			clearStack(to);
		...
		public void cyclicReferenceDetection() {
			...
			while (queue.isEmpty() == false) {
				...
				int size = memory.getBlockSize(current);
				for (int i = 0; i < size; i++)
					if (heapReferenceMemory.containsKey(current + i * 4)) {
						Integer to = heapReferenceMemory.get(current + i * 4);
			...
			for (Block block : memory.heap)
				if (visitedNodes.contains(block.start * 4) == false)
					memory.dispose(block.start * 4);
		}
	}
	...
	public void init(List<String> stringTable, List<Tac> tacs, int[] vtable, int enterPoint) {
		memory = new Memory();
			...
			synchronized (this) {
				...
				try {
					...
					case LOAD:
						...
						garbageCollector.assignHeapToStack(stack[fp + inst.opr1 / 4] + inst.opr2, resultIndex);
						break;
					case STORE:
