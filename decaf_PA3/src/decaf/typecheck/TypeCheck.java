...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitUnary(Tree.Unary expr) {
		...
		case Tree.POSTINC:
			if (expr.expr.type.equal(BaseType.ERROR)
				...
				issueError(new IncompatUnOpError(expr.expr.getLocation(), "++",
						expr.expr.type.toString()));
				expr.type = BaseType.ERROR;
			}
			break;
	...
	public void visitTernary(Tree.Ternary expr) {
		checkTestExpr(expr.bool);
		expr.expr1.accept(this);
		expr.expr2.accept(this);
		if (expr.expr1.type.equals(BaseType.ERROR) 
				|| expr.expr2.type.equals(BaseType.ERROR)
				|| expr.expr1.type.equal(expr.expr2.type)) 
			expr.type = expr.expr1.type;
		else {
			issueError(new IncompatCondError(expr.getLocation(),
					expr.expr1.type.toString(), expr.expr2.type.toString()));
			expr.type = BaseType.ERROR;
		}
	}
	...
	@Override
	public void visitTypeCount(Tree.TypeCount numinstancesExpr) {
		Class c = table.lookupClass(numinstancesExpr.className);
		numinstancesExpr.symbol = c;
		numinstancesExpr.type = BaseType.BOOL;
		if (c == null) {
			issueError(new ClassNotFoundError(numinstancesExpr.getLocation(),
					numinstancesExpr.className));
			numinstancesExpr.type = BaseType.ERROR;
		} 
		else numinstancesExpr.type = BaseType.INT;
	}
	...
	@Override
	public void visitGuardedIf(Tree.GuardedIf gifStmt) {
		for (Tree e : gifStmt.glist) e.accept(this);
	}
	
	@Override
	public void visitGuardedDo(Tree.GuardedDo gdoStmt) {
		breaks.add(gdoStmt);
		for (Tree e : gdoStmt.glist) e.accept(this);
		breaks.pop();
	}
	
	@Override
	public void visitGuarded(Tree.Guarded guarded) {
		checkTestExpr(guarded.expr);
		if (guarded.stmt != null) guarded.stmt.accept(this);
	}
