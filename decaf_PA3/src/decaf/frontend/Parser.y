...
%token INC	DEC	NUMINSTANCES	FI	DO	OD	GUARD
%token '+'  '-'  '*'  '/'  '%'  '='  '>'  '<'  '.'
%token ','  ';'  '!'  '('  ')'  '['  ']'  '{'  '}'  '?'  ':'
...
Expr            :	INC LValue
                	{
                		$$.expr = new Tree.Unary(Tree.PREINC, $2.lvalue, $1.loc);
                	}
                |	DEC LValue
                	{
                		$$.expr = new Tree.Unary(Tree.PREDEC, $2.lvalue, $1.loc);
                	}
                |	LValue INC
                	{
...
GuardedIfStmt	:	IF GuardedStmtList FI
					{
						$$.stmt = new Tree.GuardedIf($2.glist, $1.loc);
					}
				;
				
GuardedDoStmt	:	DO GuardedStmtList OD
					{
						$$.stmt = new Tree.GuardedDo($2.glist, $1.loc);
					}
				;
...
GuardedStmt		:	Expr ':' Stmt
					{
						$$.stmt = new Tree.Guarded($1.expr, $3.stmt, $2.loc);
					}
				;
